...
@section('content')
    <div class="container-fluid">
      ...
      @csrf --}}
            ...
            <a href="{{ route('showMenu', ['id' => auth()->user()->id]) }}">
                <option value="">Select Categories Here</option>
            </a>
            @foreach ($category as $cat)
                <a href="{{ route('showMenu', ['id' => auth()->user()->id, 'category_id' => $cat->id]) }}">
            ...
            <div class="row mb-5">
                @foreach ($menu as $item)
                    <div class="col mb-5 clickable" data-itemid="{{ $item->id }}">
                        <div class="card shadow-lg" style="width: 18rem;border-radius: 15px;" data-toggle="modal"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="mb-2">
                                        <img class="card-img-top" src="{{ asset('images/' . $item->id . '.jpg') }}"
                                            alt="Card image cap">
                                    </div>
                                    <div class="float-right font-weight-bold"> RM <span
                                            id="price{{ $item->id }}">{{ $item->price }}</span></div>
                                    <div class="mt-5">
                                ...
                                <div class="modal-footer">
                                    <div class="font-weight-bold mr-auto">Total : <span
                                            id="total{{ $item->id }}">0.00</span> </div>
                                    <form action="{{ route('addToCart', ['id' => auth()->user()->id]) }}" method="post">
                                        @csrf
                                        <input type="hidden" name="quantity" id="post_quant{{ $item->id }}"
                                            value="0">
